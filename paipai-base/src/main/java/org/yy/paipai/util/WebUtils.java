	...
	private static String _doPost(String url, String ctype, byte[] content, int connectTimeout, int readTimeout,
		...
		try {
			try {
				...
				PaiPaiLogger.logCommError(e, url, map.get("app_key"), map.get("method"), content);
				throw e;
			}
			try {
				...
				Map<String, String> map = getParamsFromUrl(url);
				PaiPaiLogger.logCommError(e, conn, map.get("app_key"), map.get("method"), content);
				throw e;
			}
	...
	private static String _doPostWithFile(String url, Map<String, String> params, Map<String, FileItem> fileParams,
		...
		try {
			try {
				...
				Map<String, String> map = getParamsFromUrl(url);
				PaiPaiLogger.logCommError(e, url, map.get("app_key"), map.get("method"), params);
				throw e;
			}
			...
			try {
				...
				Map<String, String> map = getParamsFromUrl(url);
				PaiPaiLogger.logCommError(e, conn, map.get("app_key"), map.get("method"), params);
				throw e;
			}
		} finally {
			if (out != null) {
